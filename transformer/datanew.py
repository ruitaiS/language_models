...
def batch(batch_size, context_len, dataset, shuffle=True):
    # Keep everything as string now
    # TODO: Convert to tokens in the language model
	# dataset = [[vocab.xft.get(token, vocab.xft["<?>"]) for token in sentence] for sentence in dataset]
	...
	if shuffle:
		reorder = np.random.permutation(len(inputs)) #torch.randperm(len(inputs))
